...
\paragraph{}
Similar to Leitner boxes, the software can calculate the ability of a user to remember a card based on the number of times they have seen it, the number of times they have forgotten it, and the number of times they have remembered it.
...
\paragraph{}
We can represent this mathematically. $0 < c < 1$ represents the confidence ($c$) of the card, where $c = 0$ means the card is completely forgotten, and $c = 1$ means the card is completely remembered. 
...
\paragraph{}
Suppose we have a user that practices a card once a day for ten days. We can plot an example of how they might remember this card, with confidence ($c$) against time in days passed since first review ($t$).

\begin{center}
  \begin{tikzpicture}
    \begin{axis}[scatter/classes={a={mark=o,draw=black}},xlabel=time in days,ylabel=confidence]
...
\paragraph{}
We need to account for these shifts in ability by pulling poorly remembered cards more often than well remembered cards, while also considering time. If we can delay reviews such that the review happens right before forgetting a card, we can maximise the boost in confidence.

\paragraph{}
We can define forgetting a card as a sudden decrease in confidence ($\Delta c < -0.5$), or confidence being below a certain threshold ($c < T$).
